...
if (isset($_POST['action'], $_POST['vacation_id']) && ($_SESSION['IsAdmin'] == 1)) {
    ...
    $updateQuery = "UPDATE employee_vacations SET status = ? WHERE vacation_id = ?";
    if ($stmt = $con->prepare($updateQuery)) {
        $stmt->bind_param('si', $action, $vacationId);
        $stmt->execute();
        
        if ($stmt->error) {
            $_SESSION['message'] = "Error updating vacation request: " . $stmt->error;
        } else {
            $_SESSION['message'] = "Vacation request " . $action . " successfully.";
        }
        $stmt->close();
    } else {
